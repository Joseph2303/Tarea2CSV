...
// @ts-nocheck
const size = parseFloat(useRoute().params.size as string)

const { data: laptops } = await useAsyncData(`screen-${size}`, () =>
  queryContent('/laptops').where({ screen_cm: size }).find()
)
</script>

<template>
  <section class="screen-page">
    <h1>Pantalla: {{ $route.params.size }} cm</h1>
